...
import com.hfstudio.guidenh.libs.micromark.HtmlTagName;
...
public final class HtmlFlow {
    ...
    private static List<Tokenizer.Event> resolveToHtmlFlow(List<Tokenizer.Event> events, TokenizeContext context) {
        ...
        if (index > 1 && Objects.equals(
            events.get(index - 2)
                .token().type,
            Types.linePrefix)) {
            // Add the prefix start to the HTML token.
            events.get(index)
                .token().start = events.get(index - 2)
                    .token().start;
    ...
    private static class StateMachine {
        ...
        private State cdataOpenInside(int code) {
            if (code == buffer.charAt(index++)) {
                effects.consume(code);
                return index == buffer.length() ? context.isInterrupt() ? ok : this::continuation
        ...
        private State tagName(int code) {
            if (code == Codes.eof || code == Codes.slash
                || code == Codes.greaterThan
                || CharUtil.markdownLineEndingOrSpace(code)) {
                if (code != Codes.slash && startTag && HtmlTagName.htmlRawNames.contains(buffer.toLowerCase())) {
                ...
                return context.isInterrupt() && !context.isOnLazyLine() ? nok.step(code)
                    : startTag ? completeAttributeNameBefore(code) : completeClosingTagAfter(code);
            }
        ...
        private State completeAttributeName(int code) {
            if (code == Codes.dash || code == Codes.dot
                || code == Codes.colon
        ...
        private State completeAttributeValueBefore(int code) {
            if (code == Codes.eof || code == Codes.lessThan
                || code == Codes.equalsTo
                || code == Codes.greaterThan
        ...
        private State completeAttributeValueUnquoted(int code) {
            if (code == Codes.eof || code == Codes.quotationMark
        ...
        private State completeAttributeValueQuotedAfter(int code) {
            if (code == Codes.slash || code == Codes.greaterThan || CharUtil.markdownSpace(code)) {
                return completeAttributeNameBefore(code);
            }
        ...
        private State completeAfter(int code) {
            ...
            return code == Codes.eof || CharUtil.markdownLineEnding(code) ? continuation(code) : nok.step(code);
        }
        ...
        private State continuation(int code) {
            ...
            if (CharUtil.markdownLineEnding(code) && (kind == Constants.htmlBasic || kind == Constants.htmlComplete)) {
                return effects.check.hook(nextBlankConstruct, this::continuationClose, this::continuationAtLineEnding)
                    .step(code);
            }
        ...
        private State htmlContinueStart(int code) {
            ...
            if (CharUtil.markdownLineEnding(code)) {
                ...
                return effects.attempt.hook(tempConstruct, this::htmlContinueStart, this::done)
                    .step(code);
            }
        ...
        private State continuationRawEndTag(int code) {
            if (code == Codes.greaterThan && HtmlTagName.htmlRawNames.contains(buffer.toLowerCase())) {
                effects.consume(code);
                return this::continuationClose;
            }
