...
public class InitializeText {
    ...
    static class TextTokenizer {
        ...
        public TextTokenizer(TokenizeContext context, Map<Integer, List<Construct>> constructs,
        ...
        return (events, context) -> {
            ...
            while (++index <= events.size()) {
                ...
                if (enter == -1) {
                    ...
                    if (index != enter + 2) {
                        events.get(enter)
                            .token().end = events.get(index - 1)
                                .token().end;
                        events.subList(enter + 2, index)
                            .clear();
                        index = enter + 2;
                    }
    ...
    static List<Tokenizer.Event> resolveAllLineSuffixes(List<Tokenizer.Event> events, TokenizeContext context) {
        ...
        while (++eventIndex <= events.size()) {
            if ((eventIndex == events.size() || events.get(eventIndex)
                .token().type.equals(Types.lineEnding)) && events.get(eventIndex - 1)
                    .token().type.equals(Types.data)) {
                var data = events.get(eventIndex - 1)
                    .token();
                ...
                if (size != 0) {
                    ...
                    token.type = eventIndex == events.size() || tabs || size < Constants.hardBreakPrefixSizeMin
                        ? Types.lineSuffix
                    ...
                    token.start = new Point(
                        ...
                        index != 0 ? bufferIndex : data.start._bufferIndex() + bufferIndex);
