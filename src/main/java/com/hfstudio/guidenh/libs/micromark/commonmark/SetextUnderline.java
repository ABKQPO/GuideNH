...
public final class SetextUnderline {
    ...
    private static List<Tokenizer.Event> resolveToSetextUnderline(List<Tokenizer.Event> events,
        ...
        while (index-- > 0) {
            if (events.get(index)
                ...
                if (events.get(index)
                    .token().type.equals(Types.paragraph)) {
                    text = index;
                }
            }
            // Exit
                ...
                if (definition == null && events.get(index)
                    .token().type.equals(Types.definition)) {
                    definition = index;
                }
            }
        }
        ...
        events.get(text)
            .token().type = Types.setextHeadingText;
        ...
        if (definition != null) {
            ...
            events.add(
                definition + 1,
                Tokenizer.Event.exit(
                    events.get(content)
                        .token(),
                    context));
            events.get(content)
                .token().end = events.get(definition)
                    .token().end;
        } else {
    ...
    private static class StateMachine {
        ...
        public StateMachine(TokenizeContext context, Tokenizer.Effects effects, State ok, State nok) {
            ...
            while (index-- > 0) {
                ...
                if (!context.getEvents()
                    .get(index)
                    .token().type.equals(Types.lineEnding)
                    && !context.getEvents()
                        .get(index)
                        .token().type.equals(Types.linePrefix)
                    && !context.getEvents()
                        .get(index)
                        .token().type.equals(Types.content)) {
                    paragraph = context.getEvents()
                        .get(index)
                        .token().type.equals(Types.paragraph);
                    break;
                }
        ...
        private State start(int code) {
            Assert.check(code == Codes.dash || code == Codes.equalsTo, "expected `=` or `-`");
        ...
        private State closingSequence(int code) {
            ...
            return FactorySpace.create(effects, this::closingSequenceEnd, Types.lineSuffix)
                .step(code);
        }
