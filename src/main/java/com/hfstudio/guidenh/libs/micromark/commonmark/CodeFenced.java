...
public class CodeFenced {
    ...
    private static class StateMachine {
        ...
        public StateMachine(TokenizeContext context, Tokenizer.Effects effects, State ok, State nok) {
            ...
            initialPrefix = tail != null && tail.token().type.equals(Types.linePrefix) ? tail.context()
                .sliceSerialize(tail.token(), true)
                .length() : 0;
        }

        private State start(int code) {
            Assert.check(code == Codes.graveAccent || code == Codes.tilde, "expected `` ` `` or `~`");
        ...
        private State sequenceOpen(int code) {
            ...
            return sizeOpen < Constants.codeFencedSequenceSizeMin ? nok.step(code)
                : FactorySpace.create(effects, this::infoOpen, Types.whitespace)
                    .step(code);
        }
        ...
        private State info(int code) {
            if (code == Codes.eof || CharUtil.markdownLineEndingOrSpace(code)) {
                ...
                return FactorySpace.create(effects, this::infoAfter, Types.whitespace)
                    .step(code);
            }
                ...
                return effects.attempt
                    .hook(
                        ...
                        effects.attempt.hook(
                            ...
                            this::after,
                            initialPrefix != 0
                                ? FactorySpace.create(effects, this::contentStart, Types.linePrefix, initialPrefix + 1)
                                : this::contentStart),
                        this::after)
                    .step(code);
            }
        ...
        class ClosingFenceStateMachine {
            ...
            private State closingSequence(int code) {
                ...
                effects.exit(Types.codeFencedFenceSequence);
                return FactorySpace.create(effects, this::closingSequenceEnd, Types.whitespace)
                    .step(code);
            }
