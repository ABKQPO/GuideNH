...
public final class Subtokenize {
    ...
    public static boolean subtokenize(List<Tokenizer.Event> events) {
        ...
        while (++index < events.size()) {
            ...
            if (index != 0 && event.token().type.equals(Types.chunkFlow)
                && events.get(index - 1)
                    .token().type.equals(Types.listItemPrefix)) {
                Assert.check(event.token()._tokenizer != null, "expected '_tokenizer' on subtokens");
                subevents = event.token()._tokenizer.getEvents();
                otherIndex = 0;

                if (otherIndex < subevents.size() && subevents.get(otherIndex)
                    .token().type.equals(Types.lineEndingBlank)) {
                    otherIndex += 2;
                }

                if (otherIndex < subevents.size() && subevents.get(otherIndex)
                    .token().type.equals(Types.content)) {
                    while (++otherIndex < subevents.size()) {
                        if (subevents.get(otherIndex)
                            .token().type.equals(Types.content)) {
                            break;
                        }

                        if (subevents.get(otherIndex)
                            .token().type.equals(Types.chunkText)) {
                            subevents.get(otherIndex)
                                .token()._isInFirstContentOfListItem = true;
                            otherIndex++;
                        }
                    }
                }
            ...
            // Exit.
                ...
                if (lineIndex != null) {
                    // Fix position.
                    event.token().end = events.get(lineIndex)
                        .token().start;
    ...
    private static Map<Integer, Integer> subcontent(List<Tokenizer.Event> events, int eventIndex) {
        ...
        var tokenizer = (token._tokenizer != null ? (token._tokenizer)
            : (context.getParser()
                .get(token.contentType)
                .create(token.start)));
        ...
        while (current != null) {
            ...
            while (events.get(++startPosition)
                .token() != current) {
                // Empty.
            }
        ...
        while (++index < childEvents.size()) {
            ...
            childEvent.isExit() && childEvents.get(index - 1)
                .isEnter()
                && childEvent.token().type.equals(
                    childEvents.get(index - 1)
                        .token().type)
