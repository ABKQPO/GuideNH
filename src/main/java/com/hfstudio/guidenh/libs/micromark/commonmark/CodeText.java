...
public class CodeText {
    ...
    private static List<Tokenizer.Event> resolveCodeText(List<Tokenizer.Event> events, TokenizeContext context) {
        ...
        if ((events.get(headEnterIndex)
            .token().type.equals(Types.lineEnding)
            || events.get(headEnterIndex)
                .token().type.equals("space"))
            && (events.get(tailExitIndex)
                .token().type.equals(Types.lineEnding)
                || events.get(tailExitIndex)
                    .token().type.equals("space"))) {
            ...
            while (++index < tailExitIndex) {
                if (events.get(index)
                    .token().type.equals(Types.codeTextData)) {
                    // Then we have padding.
                    events.get(headEnterIndex)
                        .token().type = Types.codeTextPadding;
                    events.get(tailExitIndex)
                        .token().type = Types.codeTextPadding;
                    headEnterIndex += 2;
                    tailExitIndex -= 2;
        ...
        while (++index <= tailExitIndex) {
            if (enter == null) {
                if (index != tailExitIndex && !events.get(index)
                    .token().type.equals(Types.lineEnding)) {
                    enter = index;
                }
            } else if (index == tailExitIndex || events.get(index)
                .token().type.equals(Types.lineEnding)) {
                    events.get(enter)
